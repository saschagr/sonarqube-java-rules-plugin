...
public abstract class FinalParameterTestResource {
   ...
   public void catchIt() {
      try {
         // do nothing
      } catch (RuntimeException e) { // Noncompliant [[sc=16;ec=34]] {{Use 'final' for parameter e}}
         // do nothing
      }
   }
   ...
   public void catchItMultiCatch() {
      ...
      } catch (RuntimeException | IOException exception) { // Noncompliant [[sc=16;ec=56]] {{Use 'final' for parameter exception}}
         // do nothing
      }
   }
   ...
   public void forEach() {
      for (String entry : Arrays.asList("Hallo")) { // Noncompliant [[sc=12;ec=24]] {{Use 'final' for parameter entry}}
         // do nothing
      }
   }
   ...
   public void forEachWithException() {
      for (String entry : Arrays.asList("Hallo")) { // Noncompliant [[sc=12;ec=24]] {{Use 'final' for parameter entry}}
         try {
            // do nothing
         } catch (RuntimeException e) { // Noncompliant [[sc=19;ec=37]] {{Use 'final' for parameter e}}
            // do nothing
         }
      }
   }
